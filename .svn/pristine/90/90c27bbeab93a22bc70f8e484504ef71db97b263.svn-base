...
class AclFilter extends  ActionFilter
{
    ...
    public function afterAction($action, $result)
    {
        ...
        if(\Yii::$app->adminUser->isGuest&&strtolower($actionId)!='login'){
            ...
            $this->owner->redirect(['/admin/login']);
        }else {
            if (!in_array(1, $this->owner->group_id)&&!\Yii::$app->adminUser->isGuest) {
    ...
    public function checkAcl($module,$controller, $action, $group_id)
    {
        ...
        if(!$row&&in_array(strtolower($action),['create','update','view','delete'])
            ||(strtolower($module)=='admin')&&strtolower($controller)=='default'&&strtolower($action)=='index'){
            return true;
        }
