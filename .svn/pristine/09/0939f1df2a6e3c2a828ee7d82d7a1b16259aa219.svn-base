...
namespace app\base\widgets\thinker\wangeditor\actions;
...
class Upload extends Action {
    ...
    public function run() {
        ...
        //return ['jsonrpc'=>'2.0','error'=>['code'=>'101','message'=>'fff']];
...
//                    "allowFiles" => $CONFIG['scrawlAllowFiles'], //bug
        ...
        if ($result['state'] == 'SUCCESS') {
            exit($this->getUploadBaseUrl($result['url']));
        }else
        {
            exit("error|{$result['state']}");
        }
