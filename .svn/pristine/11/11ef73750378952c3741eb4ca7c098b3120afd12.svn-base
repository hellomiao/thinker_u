...
class FileUploadUI extends BaseUpload
{
    ...
    public function run()
    {
        $files = $this->model->attributes[$this->attribute];
        if ($files) {
            $filesArr = explode(',', $files);
            $filesList = Attachments::findAll(['id' => $filesArr]);
            $filesJson = [];
            foreach ($filesList as $key => $val) {
                $deleteUrl = Url::to(['file-delete','name'=>$val->name,'id'=>$val->id]);
                $filesJson[$key] = [
                    'id' => $val->id,
                    'input_name'=>$val->input_name,
                    'name' => $val->name,
                    'size' => $val->size,
                    "url" => \Yii::getAlias('@web/static/docs') . '/' . $val->path,
                    "preview" => $val->getPreview(),
                    "deleteUrl" =>$deleteUrl,
                    "deleteType" => "POST"
                ];
        ...
        echo $this->render($this->formView, ['filesJson' => $filesJson]);
