<?php
/**
 * Created by PhpStorm.
 * User: yangchunrun
 * Date: 16/4/29
 * Time: 下午1:43
 */

namespace app\home\components;


use app\admin\models\AdminAcl;
use app\admin\models\AdminGroupAcl;
use app\home\models\UserAcl;
use app\home\models\UserGroupacl;
use yii\base\ActionFilter;

class AclFilter extends ActionFilter
{


    public function afterAction($action, $result)
    {

        $module = $this->owner->module->id;
        $controller = $this->owner->id;
        $actionId = str_replace('-', '', $action->id);

        if (\Yii::$app->user->isGuest && strtolower($actionId) != 'login') {

            $this->owner->redirect(['/login']);
        } else {
            if (!in_array(1, $this->owner->group_id) && !\Yii::$app->user->isGuest) {

                $aclCheck = $this->checkAcl($module, $controller, $actionId, \Yii::$app->user->identity->group_id);


                if (!$aclCheck) {
                    return $this->owner->render('@app/home/views/layouts/ban');
                }
            }
        }
        return parent::afterAction($action, $result); // TODO: Change the autogenerated stub
    }

    //页面权限检测显示
    public function showAcl($controller, $action)
    {
        $aclCheck = $this->checkAcl($controller, $action, \Yii::$app->user->identity->group_id);
        return $aclCheck;
    }

    //检查权限
    public function checkAcl($module, $controller, $action, $group_id)
    {

        if (in_array($action, $this->except, true)) {

            return true;
        }

        $groupAclModel = new UserGroupacl();
        $aclModel = new UserAcl();
        $myAcl = $groupAclModel->find()->select('acl_ids')->where(['group_id' => $group_id])->asArray()->all();
        $acl_ids = '';
        foreach ($myAcl as $key => $val) {
            $acl_ids .= $val['acl_ids'] . ',';
        }
        $acl_ids = substr($acl_ids, 0, -1);
        $acl_array = array_unique(explode(',', $acl_ids));
        $row = $aclModel->find()->where(['lower(module)' => strtolower($module)])->where(['lower(controller)' => strtolower($controller)])
            ->andWhere(['lower(action)' => strtolower($action)])->asArray()->one();
        if (!$row && in_array(strtolower($action), ['create', 'update', 'view', 'delete'])
            || (strtolower($module) == 'home') && strtolower($controller) == 'default' && strtolower($action) == 'index'
        ) {
            return true;
        }

        if (!empty($acl_array) && !in_array($row['id'], $acl_array)) {
            return false;
        }

        return true;
    }
}