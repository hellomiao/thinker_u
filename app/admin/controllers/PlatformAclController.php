 ...
 */
class PlatformAclController extends BaseController
{
    ...
    {
        $controllers = scandir(dirname(__DIR__) . DIRECTORY_SEPARATOR . '../home/controllers');
        $controllerId = ['' => '请选择'];
    ...
    {
        if (($model = UserAcl::findOne($id)) !== null) {
            return $model;
        } else {
    ...
    public function actionGetAction()
    {
        ...
        if ($controller) {
            ...
            $content = file_get_contents(dirname(__DIR__) . DIRECTORY_SEPARATOR .
                '../home/controllers' . DIRECTORY_SEPARATOR . $controller);
            $action = [];
